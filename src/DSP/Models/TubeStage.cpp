...
TubeStageConfig::TubeStageConfig()
    : tube(Circuit::Nonlinear::TubeParams::tubeParams12AX7())
{
...
void TubeStage::prepare(double sampleRate) noexcept
{
    if (cfg_.Ck > 0.0) {
...
float TubeStage::processSample(float sample) noexcept
{
    ...
    //
    // where Vpk = Vp − Vk,  Vgk = Vin − Vk.
    //
    // Linearising Ip around the current estimate gives the 2×2 Jacobian:
    //   J = [[ −1/Rp − gds,       gds + gm ],
    //        [       −gds,   1/Rk + Geq_k + gds + gm ]]
    //
    // (∂Ip/∂Vp = gds,  ∂Ip/∂Vk = −(gds + gm) because both Vpk and Vgk
    //  decrease when Vk increases.)
    //
