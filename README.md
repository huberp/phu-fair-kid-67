...
- **Circuit-level tube simulation** — every audio sample is solved through a nonlinear Modified Nodal Analysis (MNA) circuit that replicates the 6072 variable-mu triode gain stage at the heart of the Fairchild 670.
- **Authentic sidechain** — full-wave rectifier followed by an attack/release RC envelope follower, with the original six timing positions reproduced from the hardware.
- **Stereo link modes** — run channels independently or link them with a *Linked Max* (harder-knee) or *Linked Avg* (softer-knee) shared control voltage, matching the dual stereo/lateral-vertical modes of the original.
- **Optional oversampling** — 1×, 2×, 4× or 8× oversampling with automatic latency compensation reduces aliasing from the nonlinear triode stages.
- **Dry/wet mix** — parallel compression blending from 0 % to 100 %.
- **Input & output trim** — ±20 dB gain staging before and after the compressor core.
- **Draft / High quality modes** — trade CPU load against Newton-Raphson solver accuracy at runtime without needing to reconfigure the plug-in.
...
|---|---|---|---|
| **Input Trim** | −20 dB … +20 dB | 0 dB | Gain applied before the compressor core. Use it to drive the compressor harder or softer. |
| **Output Trim** | −20 dB … +20 dB | 0 dB | Makeup gain applied after the compressor core. |
...
| **Link Mode** | Independent / Linked Max / Linked Avg | Linked Max | Controls how the left and right sidechain envelopes are combined (see §4). |
| **Oversampling** | 1× / 2× / 4× / 8× | 1× | Oversampling factor. Higher values reduce aliasing at the cost of CPU. Latency is reported to the host automatically. |
...
| **Bypass** | Off / On | Off | Hard bypass that routes dry signal directly to output (Mix is set to 0 internally). |
...
2. Use **Input Trim** to drive the detector so that gain reduction is visible (the compressor responds to the input level relative to the tube's operating point).
3. Choose a **Timing** position — start with position 1 or 2 for transient material, 3–4 for levelling.
4. Set **Link Mode** to *Linked Max* for standard stereo bus compression. Use *Independent* when left and right sources need different amounts of compression.
5. Compensate output level with **Output Trim**.
6. Use **Mix** for parallel compression.
7. Enable **Oversampling** (2× or 4×) if you hear harsh aliasing artefacts, particularly at lower sample rates.
...
The gain stage is implemented by solving — on every audio sample — the nonlinear circuit equations of a triode triode connected as a common-cathode amplifier:
...
The smoothed CV is then clamped to a safe range (`[0, 6 V]`) before being applied to the gain stage grid.
...
│       │   ├── Fairchild/
│       │   │   ├── Fairchild670Core.{h,cpp}  # Stereo compressor core
